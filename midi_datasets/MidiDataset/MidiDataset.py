"""
Aggregated MIDI dataset: load from JSONL produced by scripts/build_midi_dataset.py.

Schema: notes (str), source (str), source_dataset (str), original_id (str).
"""
...
def load_from_directory(path: str) -> Dict[str, Dataset]:
    ...
    if not data_files:
        raise FileNotFoundError(f"No JSONL files found in {path}. Run scripts/build_midi_dataset.py first.")
